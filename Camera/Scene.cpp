...
LightIntensity localIllumination(IntersectionInfo &info,
  ...
  for (int l = 0; l < lights.size(); l++) {
    ...
    if (licht::AreaLight *areaLight =
            dynamic_cast<licht::AreaLight *>(lights[l])) {
      shadowFactor =
          shadowFactorAreaLight(areaLight, &info.point, objects, index);
    } else if (licht::SoftPointLight *pointLight =
                   dynamic_cast<licht::SoftPointLight *>(lights[l])) {
      shadowFactor =
          shadowFactorSoftPoint(pointLight, &info.point, objects, index);
    } else {
